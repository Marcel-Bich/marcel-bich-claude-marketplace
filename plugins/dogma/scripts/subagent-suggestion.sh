...
echo "  1. Spawn a review agent (a specialized reviewer if your environment lists one, else general-purpose)"
...
echo "  5. 'NO git add/commit/push - only modify files, report back to the main agent'"
...
echo ""
echo "Subagents NEVER git add/commit/push - they only modify files."
echo "The main agent does git add + commit + push after review (the shared working-tree"
echo "index stays under main control; parallel subagents would race .git/index.lock)."
echo ""
echo "Delegating: use whatever specialized agents your environment actually lists -"
echo "agent names vary per install, so never assume a specific one exists (verify the"
echo "name before spawning). Always-present built-ins to fall back on: Explore, Plan, general-purpose."
echo ""
