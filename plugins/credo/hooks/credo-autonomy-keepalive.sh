...
echo "ACTION (autonomy keep-alive): You are in full-autonomy mode (flag $FLAG is set) but NO ScheduleWakeup is set. Do NOT just end the turn. Set ScheduleWakeup NOW (chain calls for pauses over 1h) and mark the wake time with '$SCRIPT_DIR/credo-autonomy-wake-mark.sh <delaySeconds>' (same delaySeconds as the ScheduleWakeup). If the autonomous work is truly finished OR there is a showstopper / weekly hard limit: end the mode deliberately with '$SCRIPT_DIR/credo-autonomy-off.sh' - then you may stop. IMPORTANT: if a suspend-on-idle directive is set for this session, EVERY deliberate exit (finished, showstopper, or weekly hard limit) MUST first run the session-autonomous SKILL power-down sequence (drive GO items as far as buildable to done, ntfy, ~20 min veto window, then power down); a bare 'credo-autonomy-off.sh' will REFUSE (exit 1) and keep the keep-alive armed. The power-down sequence calls 'credo-autonomy-off.sh --after-suspend' as its final step; only an explicit user 'leave it on' justifies 'credo-autonomy-off.sh --override'. 'GO is not empty' with nothing actually buildable is NOT a reason to skip the suspend." >&2
exit 2
